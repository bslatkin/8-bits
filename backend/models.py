...
- LoginRecord: Record of a user who's logged in. Periodically cleaned up
    ...
    - ReadState (child): Saves the state of what the user has read on a given
        shard. Lets the UI scroll the history to that starting position when
        the user relogins in to a shard (or views a specific topic shard).
...
class PostReference(ndb.Model):
    ...
    Ensures that transactional queries on a Shard will find all associated
    posts when using the high-replication datastore.
...
class Post(ndb.Model):
    ...
    ARCHIVE_REVERSE_MAPPING = {v: k for k, v in ARCHIVE_MAPPING.items()}
